...
2. Open Grafana at [[http://localhost:3000][http://localhost:3000]].
   ...
   The panels in the top row visualize the generated data, which is published on the MQTT topics =windturbines=, =solarpanels=, =streetLights=, =households=, =offices=, and =factories=.
